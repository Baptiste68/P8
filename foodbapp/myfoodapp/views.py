...
from .models import Question, Choice, Categories, Food, foodcate
            ...
            " + str(page) + ".json"
                        ...
                        if(response.ok):
                            ...
                            is not 1 :
                                ...
                                product_name = str(jData.get('products')[i].get('product_name_fr'))
                                product_name = product_name.replace('\\', '')
                                quantity = str(jData.get('products')[i].get('quantity'))
                                quantity = quantity.replace('\\', '')
                                dangers = str(jData.get('products')[i].get('traces'))
                                dangers = dangers.replace('\\', '')
                                stores = str(jData.get('products')[i].get('stores'))
                                stores = stores.replace('\\', '')
                                nutri_score = str(jData.get('products')[i].get('nutrition_grades_tags')[0])
                                ...
                                if not Food.objects.filter(name_food = product_name).exists():
                                    ...
                                    id_category = Categories.objects.only('id').get(name_categories=category).id

                                    if not foodcate.objects.filter(Food_id = my_id, Categories_id = id_category).exists():
                                        print("food id: "+str(my_id)+" cate : "+str(id_category))
                                        my_insert = foodcate(Food_id = Food.objects.get(id = my_id), Categories_id = Categories.objects.get(id = id_category))
                                        my_insert.save()
...
def searching_cat(product):
    ...
    id_cat = foodcate.objects.only('Categories_id_id').get(Food_id_id=id_prod).Categories_id_id
    name_cat = Categories.objects.only('name_categories').get(id=id_cat).name_categories
    print(name_cat)
    return name_cat

def get_better_food(product, category):
    nutri_score = Food.objects.only('nutri_score_food').get(name_food=product).nutri_score_food
    nutri_score = ord(nutri_score)

    id_category = Categories.objects.only('id').get(name_categories=category).id
    candidate_ids = foodcate.objects.filter(Q(Categories_id_id = id_category)).values('Food_id_id')

    results = []

    for id in candidate_ids:
        candidate_score = Food.objects.only('nutri_score_food').get(id=id['Food_id_id']).nutri_score_food
...
class SearchView(generic.ListView):
    ...
    def get(self, request):
        ...
        my_result = []
        for id in list_id:
            temp=Food.objects.filter(id=id['Food_id_id']).values('name_food', 'nutri_score_food', 'id')
            my_result.append({'name_food' : temp[0]['name_food'], 'nutri_score_food' : temp[0]['nutri_score_food'], 'id' : temp[0]['id']})

        return render(request, self.template_name, {'aliment' : aliment, 'category' : category, 'my_result' : my_result})
...
class ProductView(generic.ListView):
    ...
    def get(self, request):
        ...
        temp=Food.objects.filter(id=product_id).values('name_food', 'nutri_score_food', 'quantity_food', 'link_food')
